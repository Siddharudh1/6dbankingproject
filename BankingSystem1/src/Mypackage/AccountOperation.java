...
public class AccountOperation {
	...
	public static void addMoney(String username,double amount) {
		for(Account ele : L1) {
			if(ele.getUsername().equals(username)) {
				...
				System.out.println("Amount "+amount+" credited ....Total balance : "+ele.getBalance());
			}
		}
	...
	public static void viewBalance(String username,long pin) {
		for(Account ele : L1) {
			if(ele.getUsername().equals(username) || pin == ele.getPin()){
	...
	public static void withdrawMoney(String username,long pin,double amount) {
		for(Account ele : L1) {
			if(ele.getUsername().equals(username) && pin == ele.getPin() && amount > ele.getBalance()){
				double bal = ele.getBalance();
				ele.setBalnce(bal - amount);
